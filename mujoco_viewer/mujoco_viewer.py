...
class MujocoViewer(Callbacks):
    ...
    def set_grid_divisions(
        self, x_div: int, y_div: int, x_axis_time: float = 0.0, override=False
    ):
        if override is False:
            assert (
                x_axis_time >= self.model.opt.timestep * 50
            ), ("Set [x_axis_time] >= [self.model.opt.timestep * 50],",
                " inorder to get a suitable sampling rate")
            ...
            for i in range(0, len(f_render_list)):
                self.add_marker(
                    ...
                    size=[
                        arrow_radius,
                        arrow_radius,
                        self.data.actuator(f_render_list[i][1]).force
                        * force_scale,
                    ],
                    rgba=rgba_list,
    ...
    def render(self):
        ...
        def update():
            ...
            with self._gui_lock:
                ...
                if not self._paused and not self._hide_graph:
                    self.sensorupdate()
